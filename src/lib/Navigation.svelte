...
>
	...
	<ul class="relative hidden flex-row text-xl sm:flex">
		...
		<span
			...
			style={`transform: translateX(${activeItem?.offsetLeft !== hoveredElement?.offsetLeft ? hoveredElement?.offsetLeft : activeItem?.offsetLeft}px); width: ${Math.max(hoveredElement?.offsetWidth ?? 0, activeItem?.offsetWidth)}px;`}
		></span>
	</ul>
